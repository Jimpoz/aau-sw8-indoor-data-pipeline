...
def resolve_model_selection(
    ...
    selected_profile = requested_profile or active_profile

    if selected_profile:
        if selected_profile not in profiles:
            available_profiles = ", ".join(sorted(profiles)) or "none"
            raise ValueError(
                f"Unknown model profile '{selected_profile}'. "
                f"Available profiles: {available_profiles}."
            )

        resolved_path, class_config_path, confidence_threshold = profiles[selected_profile]
...
def _load_model_profiles(
    config_path: str | Path | None,
) -> tuple[dict[str, tuple[Path, Path | None, float]], str | None]:
    ...
    profiles: dict[str, tuple[Path, Path | None, float]] = {}
    ...
    active_profile = parser.get("selection", "active", fallback=None)
...
def load_model_profiles(
    ...
    return _load_model_profiles(config_path)
...
def resolve_config_profile(
    ...
    model_path, _, _ = profiles[selected_profile]
