...
def main() -> None:
    ...
    profile_name, configured_model_path = resolve_config_profile(
        config_path=Path(args.config),
        requested_profile=args.profile,
    )
    if configured_model_path.name not in YOLO11_DETECTION_ASSETS:
        available_models = ", ".join(YOLO11_DETECTION_ASSETS)
        raise ValueError(
            f"Profile '{profile_name}' points to unsupported model asset "
            f"'{configured_model_path.name}'. Supported YOLO11 assets: {available_models}."
        )

    model_path = download_yolo11_detection_model(
        model_name=configured_model_path.name,
        output_dir=configured_model_path.parent,
    )
    print(model_path)
