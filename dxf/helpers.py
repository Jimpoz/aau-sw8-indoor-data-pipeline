"""DXF / DWG floor-plan import helpers."""
import io
import json
import shutil
import subprocess
...
def _convert_dwg_to_dxf(dwg_bytes: bytes) -> bytes:
    ...
    raise FileNotFoundError(
        "No DWG converter is installed in the indoor data pipeline container. "
        "Install the LibreDWG package (provides `dwgread`) or supply ODA's "
        "ODAFileConverter binary, or upload an already-converted .dxf instead."
    )
